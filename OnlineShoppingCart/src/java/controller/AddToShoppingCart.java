...
public class AddToShoppingCart extends HttpServlet {
    ...
    // get parametot --> request & response 
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for(String MVname : checkbArray){   
            for(Products pd : pdList){ 
                ...
                if(MVname.trim().equals(pd.getMovie()) && !request.getParameter(MVname.trim()).equals("")){ 
                    ...
                    totalPrice = Calculator.totalCalculator(totalPrice, unitPrice); // Total price (all product)
        ...
        while(AttrList.hasMoreElements()){
            ...
            if(singleAttr.contains("_Quantity")){
                session.setAttribute(singleAttr.replace("_Quantity", ""), request.getAttribute(singleAttr)); // singleAttr --> MVname_Quantity
                //return session's attr (name : movie , value : unit)
            }
        } 
